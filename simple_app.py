...
with app.app_context():
    ...
    try:
        ...
        for project in projects:
            print(f"Projekt '{project.title}': Kategorie={project.category}, Icon={project.icon}, Position={project.position}")
            
            # Kategorie basierend auf dem Icon zuweisen, falls noch nicht gesetzt
            if project.category is None and project.icon:
                icon_name = project.icon.replace('fas ', '').replace('far ', '').replace('fab ', '')
                for key, value in icon_to_category.items():
                    if key in icon_name:
                        project.category = value
                        db.session.add(project)
            
            # Icon basierend auf der Kategorie zuweisen, falls noch nicht gesetzt
            if project.icon is None and project.category:
                if project.category in category_to_icon:
                    project.icon = f"fas {category_to_icon[project.category]}"
                    db.session.add(project)
            
            # Standardwerte für Position setzen, falls nicht gesetzt
            if project.position is None:
                project.position = 999
                db.session.add(project)
        
        db.session.commit()
    except Exception as e:
        print(f"Fehler bei der Überprüfung der Projektkategorien: {str(e)}")
    
    # Admin-Benutzer erstellen, falls keiner existiert
    if User.query.filter_by(username=app.config['ADMIN_USERNAME']).count() == 0:
        admin_user = User(
    ...
    #             'description': 'Ein Projekt zur Förderung von Bildung für benachteiligte Frauen.',
    #             'icon': 'fas fa-graduation-cap',
    #             'color': '#4CAF50',
    #             'details': '''
    #                 Unser Bildungsprojekt bietet kostenlose Workshops und Kurse für Frauen, die ihre beruflichen Fähigkeiten verbessern möchten. 
    #                 Von Sprachkursen bis hin zu technischen Schulungen – wir helfen Frauen, ihre Karrierechancen zu verbessern.
                    
    #                 Wir vermitteln praktische Fähigkeiten und fördern das Selbstvertrauen unserer Teilnehmerinnen. Durch individuelle Betreuung 
    #                 und Netzwerkmöglichkeiten schaffen wir nachhaltige Perspektiven für eine bessere Zukunft.
    #             ''',
    ...
    #             'description': 'Ein Projekt zum Schutz unserer Umwelt und zur Förderung nachhaltiger Praktiken.',
    #             'icon': 'fas fa-leaf',
    #             'color': '#2196F3',
    #             'details': '''
    #                 Mit unserer Umweltschutz-Initiative engagieren wir uns für den Erhalt natürlicher Ressourcen und die Verbesserung unserer Umwelt. Wir 
